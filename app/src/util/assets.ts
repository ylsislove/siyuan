...
const loadThirdIcon = (iconURL: string, data: IAppearance) => {
    addScript(iconURL, "iconDefaultScript").then(() => {
        if (!["ant", "material"].includes(data.icon)) {
            ...
            addScript(`${Constants.PUBLIC_PATH}/appearance/icons/${data.icon}/icon.js?v=${data.iconVer}`, "iconScript");
        }
    });
};
...
export const loadAssets = (data: IAppearance) => {
    ...
    const themeScriptAddress = `${Constants.PUBLIC_PATH}/appearance/themes/${data.mode === 1 ? data.themeDark : data.themeLight}/theme.js?v=${data.themeVer}`;
    ...
    const iconURL = `${Constants.PUBLIC_PATH}/appearance/icons/${["ant", "material"].includes(data.icon) ? data.icon : "material"}/icon.js?v=${Constants.SIYUAN_VERSION}`;
...
export const initAssets = () => {
    ...
    if (!emojiElement && !window.siyuan.config.appearance.nativeEmoji && !isMobile()) {
        addScript("${Constants.PUBLIC_PATH}/appearance/emojis/twitter-emoji.js?v=1.0.1", "emojiScript").then(() => {
            if (loadingElement) {
